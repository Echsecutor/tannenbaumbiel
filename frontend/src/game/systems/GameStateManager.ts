...
export class GameStateManager {
    ...
    constructor(scene: Phaser.Scene, networkSystem: NetworkSystem, isOffline: boolean = false) {
        this.scene = scene
    ...
    showGameOver(onRestart: () => void) {
        ...
        const restartText = this.scene.add.text(centerX, centerY + 100, 'Klicken zum Neustart', {
            fontSize: '24px',
            color: '#ffffff',
    ...
    showVictory(onNextLevel: () => void, onMenu: () => void) {
        ...
        const victoryText = this.scene.add.text(centerX, centerY, 'Sieg!', {
            fontSize: '64px',
            color: '#27ae60',
            backgroundColor: '#2c3e50',
            padding: { x: 20, y: 10 }
        ...
        const continueText = this.scene.add.text(centerX, centerY + 100, 'Nächste Welt', {
            fontSize: '24px',
            color: '#ffffff',
        ...
        const menuText = this.scene.add.text(centerX, centerY + 150, 'Zurück zum Menü', {
            fontSize: '18px',
            color: '#ffffff',
    ...
    private setRejoinFlags() {
        ...
        if (!this.scene.registry.get('originalRoomName') || !this.scene.registry.get('originalUsername')) {
