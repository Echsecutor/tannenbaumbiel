...
export class EnemySystem {
    ...
    private createEnemyAnimations() {
        ...
        if (!this.scene.anims.exists('pink_enemy_idle_anim')) {
            this.scene.anims.create({
                ...
                frames: this.scene.anims.generateFrameNumbers('pink_enemy_idle', { start: 0, end: 3 }),
                frameRate: 6,
                repeat: -1
            })
        }
    ...
    createEnemy(x: number, y: number, enemyType: string, chunkIndex: number): Phaser.Physics.Arcade.Sprite {
    ...
    hitEnemy(enemy: any, damage: number = 25): { destroyed: boolean, health: number } {
    ...
    updateNetworkEnemies(enemyStates: any[], platforms: Phaser.Physics.Arcade.StaticGroup, player: Phaser.Physics.Arcade.Sprite, projectiles: Phaser.Physics.Arcade.Group) {
        ...
        for (const enemyState of enemyStates) {
            ...
            if (!sprite) {
                try {
                    ...
                    sprite = this.scene.physics.add.sprite(enemyState.x, enemyState.y, texture)
                    sprite.setDepth(12)
                    
                    if (!sprite || !sprite.body) {
                        console.error(`❌ Failed to create enemy sprite ${enemyState.enemy_id}`)
                        continue
                    }
                    ...
                    this.scene.physics.add.overlap(player, sprite, (p: any, e: any) => this.hitEnemy(e), undefined, this.scene)
                    this.scene.physics.add.overlap(projectiles, sprite, (pr: any, e: any) => this.hitEnemy(e), undefined, this.scene)
            ...
            if (sprite && sprite.body) {
                try {
                    ...
                    if (Math.abs(enemyState.velocity_x) > 10) {
                        const walkAnim = enemyState.enemy_type === 'pink_boss' ? 'pink_enemy_walk_anim' : 'enemy_walk_anim'
                        sprite.play(walkAnim, true)
                    } else {
                        const idleAnim = enemyState.enemy_type === 'pink_boss' ? 'pink_enemy_idle_anim' : 'enemy_idle_anim'
