...
export class NetworkSystem {
    ...
    broadcastGameState(gameState: any) {
        if (!this.networkManager || !this.myPlayerId || !this.roomJoinConfirmed) return
    ...
    private updateNetworkPlayer(playerState: any) {
        ...
        if (!sprite) {
            try {
                sprite = this.scene.physics.add.sprite(playerState.x, playerState.y, 'player_idle')
                if (!sprite) return
    ...
    updateNetworkProjectiles(projectileStates: any[], platforms: Phaser.Physics.Arcade.StaticGroup, enemies: Phaser.Physics.Arcade.Group) {
        ...
        for (const projectileState of projectileStates) {
            ...
            if (!sprite) {
                try {
                    sprite = this.scene.physics.add.sprite(projectileState.x, projectileState.y, 'projectile')
                    ...
                    // Add collisions
                    this.scene.physics.add.overlap(sprite, enemies, (pr: any, e: any) => {
                        pr.destroy()
                        // Handle enemy hit
                    }, undefined, this.scene)
                    this.scene.physics.add.collider(sprite, platforms, (pr: any) => {
                        pr.destroy()
                    }, undefined, this.scene)
    ...
    addPlatformCollisionForNetworkPlayers(platforms: Phaser.Physics.Arcade.StaticGroup) {
        for (const [playerId, sprite] of this.networkPlayers) {
