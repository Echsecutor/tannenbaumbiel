...
export class PhysicsSystem {
    ...
    setupCollisions(
        ...
        this.scene.physics.add.overlap(player, enemies, onPlayerEnemyHit, undefined, this.scene)
        
        // Projectile-enemy collisions
        this.scene.physics.add.overlap(this.projectiles, enemies, onProjectileEnemyHit, undefined, this.scene)
        
        // Projectile-platform collisions
        this.scene.physics.add.collider(this.projectiles, platforms, onProjectilePlatformHit, undefined, this.scene)
    }
    ...
    enablePhysics(gameObject: Phaser.GameObjects.GameObject): Phaser.Physics.Arcade.Body | null {
    ...
    addCollider(
        ...
        return this.scene.physics.add.collider(object1, object2, collideCallback, processCallback, callbackContext)
    }
    ...
    addOverlap(
        ...
        return this.scene.physics.add.overlap(object1, object2, overlapCallback, processCallback, callbackContext)
    }
    ...
    getDistance(object1: any, object2: any): number {
        return Phaser.Math.Distance.Between(object1.x, object1.y, object2.x, object2.y)
    }

    // Get angle between two objects
    getAngle(object1: any, object2: any): number {
        return Phaser.Math.Angle.Between(object1.x, object1.y, object2.x, object2.y)
    }
    ...
    cleanupProjectiles(worldBounds: Phaser.Geom.Rectangle) {
        this.projectiles.children.entries.forEach((projectile: any) => {
            if (projectile.active && !worldBounds.contains(projectile.x, projectile.y)) {
                projectile.destroy()
            }
        })
    }
